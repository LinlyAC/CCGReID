...
def hard_example_mining_arg(dist_mat, is_pos, is_neg):
    ...
    dist_ap, ap_index = torch.max(dist_mat * is_pos, dim=1)
    # `dist_an` means distance(anchor, negative)
    # both `dist_an` and `relative_n_inds` with shape [N]
    dist_an, an_index = torch.min(dist_mat * is_neg + is_pos * 1e9, dim=1)
    return ap_index, an_index
